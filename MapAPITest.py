...
def TianTuMapAPI(key):
    ...
    # 1. 检测 SDK 是否能正常加载
    try:
        sdk_url = f"http://api.tianditu.gov.cn/api?v=4.0&tk={key}"
    ...
    try:
        ...
        r_api = requests.get(api_url, headers=Headers, params=params, timeout=5)
        data = r_api.json()
        if data.get("status") == "0":
...
if __name__ == "__main__":
    ...
    if Tencent_AK:
        ...
        print()
    if TianTu_TK:
        results = TianTuMapAPI(TianTu_TK)
        print("天地图API密钥检测结果:")
        for k, v in results.items():
            print(f"{k}: {v}")
        print()
